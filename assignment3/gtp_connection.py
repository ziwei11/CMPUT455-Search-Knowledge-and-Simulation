...
class GtpConnection:
    ...
    def policy_cmd(self, args):
        ...
        if (args[0] == "random"):
            ...
            self.respond("Invalid. The argument policytype should be either random or rule_based.")
            return
        self.respond("The policytype is " + args[0])

    def policy_moves_cmd(self, args):
        # This command prints the set of moves considered by the simulation policy,
        # for the current player in the current position, in the format
        assert(len(args) == 0)
        ...
        if (self.policy_is_random):
            movetype = "Random"
            movelist = self.board.get_empty_points()
        else:
            movetype, movelist = self.board.rule_based_simulation(self.board.current_player)

        # = MoveType movelist 
        # Where MoveType is one of: {Win, BlockWin, OpenFour, BlockOpenFour, Random}.
        # Movelist is an alphabetically sorted list of moves
        # (same sorting order as implemented in gogui-rules_legal_moves)
        str_movelist=[]
        for move in movelist:
            move_coordinate = point_to_coord(move, self.board.size)
            str_movelist.append(format_point(move_coordinate))

        str_movelist.sort()
        self.respond(movetype + " " + " ".join(str_movelist))
