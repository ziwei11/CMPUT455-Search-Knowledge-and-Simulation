...
class GoBoard(object):
    ...
    def rule_based_simulation(self, color):
        self.lines = []
        map(self.lines.extend(self.rows), self.rows)
        map(self.lines.extend(self.cols), self.cols)
        map(self.lines.extend(self.diags), self.diags)
        if len(self.Win(color)) > 0:
            return ("Win", self.Win(color))
        if len(self.BlockWin(color)) > 0:
            return ("BlockWin", self.BlockWin(color))
        if len(self.OpenFour(color)) > 0:
            return ("OpenFour", self.OpenFour(color))
        if len(self.BlockOpenFour(color)) > 0:
            return ("BlockOpenFour", self.BlockOpenFour(color))
        return ("Random", self.get_empty_points())

    def Win(self, player):
        """
        if you can win directly, in one move, then only consider one of the winning moves.
        """
        opponent_player = GoBoardUtil.opponent(player)
        moves = []
        for k in self.lines:
    ...
    def BlockWin(self, player):
        """
        if the opponent can win directly, then only play a move that blocks the win. 
        For example, OO.OO can be blocked by a move OOXOO.
        Even if you cannot prevent the win, as in the case .OOOO., 
        or when there is more than one open four on the board, you should generate a blocking move.
        """
        self.current_player = GoBoardUtil.opponent(player) 
        moves = self.Win(self.current_player)
    ...
    def OpenFour(self, player):
        ...
        for k in self.lines:
            for i in range(len(k) - 5): 
                if self.get_color(k[i]) != EMPTY or self.get_color(k[i + 5]) != EMPTY:
                    continue
                position = -1
                for j in k[i + 1: i + 5]:
                    color = self.get_color(j)
    ...
    def BlockOpenFour(self, player):
        """
        play a move that prevents the opponent from getting an open four. 
        For example, the situation ..OOO.. can be blocked by moves .XOOO.. or ..OOOX.. 
        Note that all moves that block an open four should be generated. 
        Specifically, in patterns .OO.O. and X.OOO.. all three moves work to block the open four and should be generated.
        """
