...
class GtpConnection:
    def __init__(self, go_engine, board, debug_mode=False):
        ...
        self.argmap = {
            ...
            "timelimit": (1, 'Usage: timelimit {interger seconds(1<=seconds<=100)}'),
            "solve": (0, 'No arguments')
        }
    ...
    def boardsize_cmd(self, args):
        ...
        """
        s = int(args[0])
        if s != self.board.size:
            self.hasher = ZobristHash(s)
            self.transposition = TranspositionTable()
        self.reset(s)
        self.respond()
    ...
    def timelimit_cmd(self, args):
        if len(args) == 1 and args[0].isdigit():
            seconds = int(args[0])
            if seconds >= 1 and seconds <= 100:
                self.maximum_time = seconds
        self.respond()

    def solve_cmd(self, args):
        outcome, move = self.solve()
        if move == None:
            self.respond("{}".format(outcome))
        else:
            coordinate = point_to_coord(move, self.board.size)
            move = format_point(coordinate)
            self.respond("{} {}".format(outcome, move))

    def time_handler(self, signal, frame):
        raise TimeoutError

    def solve(self):
        signal.signal(signal.SIGALRM, self.time_handler)
        signal.alarm(self.maximum_time)
        copied_board = self.board.copy()
        try:
            value, move = call_alphabeta(copied_board, self.transposition, self.hasher)
            if value == 0:
                return "draw", move
            elif value > 0:
                ...
                if GoBoardUtil.opponent(self.board.current_player) == BLACK:
                    return 'b', None
                if GoBoardUtil.opponent(self.board.current_player) == WHITE:
                    return 'w', None
        except TimeoutError:
            return "unknown", None
        finally:
