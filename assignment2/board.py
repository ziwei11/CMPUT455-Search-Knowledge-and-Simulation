...
###FIX BOARD.PY###
    ...
    #The player does not do any move
    def undo_move(self, move):
        self.board[move] = EMPTY
        self.current_player = GoBoardUtil.opponent(self.current_player)

    #The player gets the best move
    def best_move(self):
        move = self.get_empty_points()
        return sorted(move, key = self.get_score, reverse = True)

    #Determine if the game is over
    def game_over(self):
        winner = self.detect_five_in_a_row()
        #If there is no empty points in the board, the game ends
        if self.get_empty_points().size == 0:
            return True
        #If there is a winner of the game, the game ends
...
###FIX BOARD.PY###
    ...
    #get the score of the movements
    ...
    def evaluate(self):
        #check winner
        assert self.detect_five_in_a_row() != self.current_player
        if self.detect_five_in_a_row() != EMPTY:
            return -1000000
        return self.hauristic()

    def hauristic(self):
        score = 0
        for i in (self.diags + self.rows + self.cols):
            #For every five lines in the board
            #we calculate how many stones are there for each color
            for j in range(len(i) - 5):
                play_A = 0      #The current player
                play_B = 0      #The opponent of the current player
                for n in i[j : (j + 5)]:
                    if self.board[n] == self.current_player:
                        play_A += 1
                    elif self.board[n] == GoBoardUtil.opponent(self.current_player):
                        play_B += 1
                if play_A < 1 or play_B < 1:
                    score += 10 ** play_A - 10 ** play_B
        return score
